...
Node* makeNodeString(label_t label, char* val){
  Node* node = makeNode(label);
  if (val != NULL)
    strcpy(node->ident,val);
  return node;
}
...
void printTree(Node *node) {
  ...
  if(node->label == id || node->label == num || node->label == character || node->label == divstar || node->label == compare || node->label == order || node->label == type) 
    printf(": %s", node->ident);
